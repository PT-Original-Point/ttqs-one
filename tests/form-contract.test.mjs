...
for (const token of expectedTokens) {
  test(`form contract contains ${token}`, () => assert.ok((forms + router).includes(token)));
}
...
test('survey dedupe checks source_ref', () => assert.match(survey, /'source_ref'/));
test('runtime survey is sample-only', () => assert.match(survey, /not formal REAL evidence/));

for (const code of ['CLARITY', 'RELEVANCE', 'SAFETY', 'PRACTICE', 'OVERALL']) {
  test(`reaction dimension list contains ${code}`, () => assert.ok(forms.includes(`'${code}'`)));
}
