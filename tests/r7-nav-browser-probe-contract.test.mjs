...
test('R7 browser probe implements every human-specified a-g route check without threshold reduction', () => {
  ...
  for (const strictDocumentCheck of ['everyCardHasTitle','everyCardHasOpenDocument','everyCardHasCanonicalArtifactRoute']) assert.ok(probe.includes(strictDocumentCheck),strictDocumentCheck);
});

test('R7 browser probe records actual DOM evidence for every route', () => {
  for (const marker of ['actualUrl','resolvedHref','domEvidence','cardDataIndicator','linkDataIndicator','hrefAttribute','resolvedHref','targetAttribute','linkOuterHTMLSha256','cardOuterHTMLSha256','firstChineseDocumentTitle','firstCanonicalOpenDocument','perCard']) assert.ok(probe.includes(marker),marker);
...
test('browser workflow permanently publishes complete 19+19 rows before enforcing 38/38', () => {
  ...
  assert.ok(workflow.includes('for VIEWPORT in desktop mobile'));
  assert.ok(workflow.includes('r7-nav-$VIEWPORT.json'));
  assert.ok(workflow.includes('r7-nav-$VIEWPORT.md'));
