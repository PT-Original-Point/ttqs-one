...
function ttqsRawSubmission_(sheetId, rowNumber) {
  ...
  return {
    ...
    rawRef: 'FORM_SUITE:' + formId + ':' + sheetId + ':' + fingerprint
  };
}
...
function ttqsHandleRawSubmissionUnlocked_(sheetId, rowNumber, isRetry) {
  return ttqsHandleRawObjectUnlocked_(ttqsRawSubmission_(sheetId, rowNumber), isRetry);
}
...
function ttqsHandleRawRef_(sheetId, rawRef, isRetry) {
  return ttqsWithScriptLock_(function() {
    return ttqsHandleRawObjectUnlocked_(ttqsFindRawSubmissionByRef_(sheetId, rawRef), isRetry);
  });
}
