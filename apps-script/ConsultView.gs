...
function ttqsWebFormLaunchHtml_(forms, health) {
  ...
  var cards = forms.map(function(item) {
    ...
    return '<article class="launch-card"><div><span class="launch-state">' + ttqsWebEscape_(item.status) + '</span><h3>' + ttqsWebEscape_(item.label) + '</h3><p>使用 Google Forms 原生填答頁面；只使用 SAMPLE 示範資料。</p></div>' + action + '</article>';
  }).join('');
  var healthHtml = health
    ...
    : '';
  return '<section><div class="section-heading"><h2>顧問示範流程入口</h2><p>填答仍走 Google Forms 原生介面，不繞過既有 TEST 處理流程。</p></div>' + healthHtml + '<div class="launch-grid">' + cards + '</div></section>';
}
