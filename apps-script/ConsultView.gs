...
function ttqsWebHealthSummary_() {
  try {
    ...
    var total = health && health.checks ? health.checks.length : 0;
    var failed = health && health.failed ? health.failed.length : 0;
    return {
...
function ttqsWebModuleCardsHtml_(modules) {
  ...
  return '<section><div class="section-heading"><h2>日常辦訓工作</h2><p>這些卡片是目前測試／示範能力（TEST／SAMPLE）投影，不代表正式營運已完成。</p></div>' +
...
function ttqsWebFormLaunchHtml_(forms, health) {
  ...
  var cards = forms.map(function(item) {
    var action = item.url
      ? '<a class="launch-action" target="_blank" rel="noopener noreferrer" href="' + ttqsWebEscape_(item.url) + '">開啟填答</a>'
      : '<span class="launch-action disabled">' + ttqsWebEscape_(item.status) + '</span>';
  ...
  var healthHtml = health
    ? '<div class="health-summary"><strong>系統狀態：' + ttqsWebEscape_(health.state) + '</strong><span>自動檢查 ' + ttqsWebEscape_(health.total) + ' 項；需檢查 ' + ttqsWebEscape_(health.failed) + ' 項</span></div>'
    : '';
...
function ttqsRenderWebAppHtml_(model) {
  ...
  var viewNote = externalPreview
    ? '目前僅為外部唯讀畫面預覽；尚未建立正式外部部署，也不讀取正式個資（REAL）。'
    : '目前為測試／示範資料（TEST／SAMPLE）內部畫面；所有正式資料寫入（REAL）仍停用。';
  ...
  return '<!doctype html><html lang="zh-Hant-TW"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    ...
    '<p class="footnote">目前畫面直接讀取既有測試／示範（TEST／SAMPLE）核心資料。外部預覽不等於正式唯讀快照；正式評核發布仍需獨立的唯讀快照與核准流程。</p>' +
    '</main></body></html>';
}
...
function doGet(e) {
  try {
    ...
    return HtmlService.createHtmlOutput(ttqsWebErrorHtml_())
      .setTitle('TTQS ONE')
      .addMetaTag('viewport', 'width=device-width, initial-scale=1');
  }
}
