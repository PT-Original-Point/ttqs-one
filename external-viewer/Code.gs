...
var TTQS_EXTERNAL_SUMMARY_ = {
  ...
  '來源': '受控文件、clean SAMPLE-002 與 TEST 核心權威 readback 的去識別靜態投影；外部 Viewer 不在執行期直連核心',
  '來源更新時間': '2026-08-19 17:15 台北時區（Asia/Taipei）；CLEAN_FIXTURE_2／TEST readback 基線',
  '官方語意來源': '勞動部勞動力發展署現行 TTQS 訓練機構版評核表與查核佐證文件資料表之顧問導航摘要；正式評核仍以官方最新文件為準',
  '指標範圍': '1–19；19/19 僅代表介面與查驗路徑覆蓋，不代表正式可評事證已成立',
  '四類TEST表單': 'NEEDS／REGISTRATION／REACTION／FOLLOWUP30；目前靜態 readback 共 14 筆 Observation，4/4 類別均有 ACCEPTED 來源',
  '17–19 狀態': '系統能力與 TEST／SAMPLE 方法可查驗；協會目前尚無 REAL Outcome，不宣稱正式達成',
...
var TTQS_EXTERNAL_INDICATORS_ = [
  ['1','規劃','訓練機構未來經營方向及目標之訂定','1','系統能力可展示；正式可評事證 NOT_FORMAL_READY','2026-08-19 17:15:00'],
  ['2','規劃','對外明確的訓練政策','1','系統能力可展示；正式可評事證 NOT_FORMAL_READY','2026-08-19 17:15:00'],
  ['3','規劃','明確的 PDDRO 訓練課程及明確的核心訓練類別','1','系統能力可展示；正式可評事證 NOT_FORMAL_READY','2026-08-19 17:15:00'],
  ['4','規劃','訓練品質管理的系統化文件資訊','1','系統能力可展示；CURRENT／WORKING 不等於 APPROVED／EFFECTIVE','2026-08-19 17:15:00'],
  ['5','規劃','訓練規劃及經營目標的連結性','1','系統能力可展示；正式可評事證 NOT_FORMAL_READY','2026-08-19 17:15:00'],
  ['6','規劃','訓練機構的行政管理能力及訓練主管相關職能','1','A/B/C 為治理模型；真人職能事證尚未形成','2026-08-19 17:15:00'],
  ['7','設計','訓練需求相關的職能分析及應用','2','clean SAMPLE-002 與 TEST needs readback 可展示；非 REAL 需求','2026-08-19 17:15:00'],
  ['8','設計','訓練方案的系統設計','1','clean SAMPLE-002 方法可展示；非 REAL 課程方案','2026-08-19 17:15:00'],
  ['9','設計','利益關係人的參與過程','1','clean SAMPLE-002 模擬參與；非真人治理核准','2026-08-19 17:15:00'],
  ['10','設計','訓練資源的採購程序及甄選標準','1','clean SAMPLE-002 方法可展示；受控資源原件仍有缺口','2026-08-19 17:15:00'],
  ['11','設計','訓練計畫及目標需求的結合','2','clean SAMPLE-002＋TEST needs 可追溯；非 REAL','2026-08-19 17:15:00'],
  ['12','執行','訓練內涵按計畫執行的程度','2','12a–e 方法可查驗；部分受控原件仍有缺口','2026-08-19 17:15:00'],
  ['13','執行','提供學習成果移轉的建議或協助','2','clean SAMPLE-002＋TEST follow-up 可展示；非 REAL 成果','2026-08-19 17:15:00'],
  ['14','執行','訓練資料分類與建檔及管理資訊系統化','1','系統化索引能力可展示；不可替代 REAL 訓練檔案','2026-08-19 17:15:00'],
  ['15','查核','評估報告及定期性綜合分析','2','clean SAMPLE-002 方法＋TEST reaction 可展示；正式結案原件尚未形成','2026-08-19 17:15:00'],
  ['16','查核','管控及異常矯正處理','1','S3 CONTROL trace 可展示；不是辦訓異常事證','2026-08-19 17:15:00'],
  ['17','成果','訓練成果評估的多元性和完整性','2','系統能力可展示；正式成果 FORMAL_BLOCKED_NEEDS_REAL','2026-08-19 17:15:00'],
  ['18','成果','訓練系統的一般性功能─目標客戶及學員的評價','1','正式評價 FORMAL_BLOCKED_NEEDS_REAL','2026-08-19 17:15:00'],
  ['19','成果','訓練系統的市場功能─目標市場及顧客的價值創造','1','正式市場價值 FORMAL_BLOCKED_NEEDS_REAL','2026-08-19 17:15:00']
];
...
var TTQS_EXTERNAL_OFFICIAL_FOCUS_ = [
  ...
  ['7','確認課程設計前是否有客戶／市場需求確認、職能需求與落差分析，以及方法與紀錄。',[], '從 TEST 需求表單 Observation 下鑽，再接 clean SAMPLE-002 落差分析。'],
  ...
  ['12','確認實際訓練內涵按計畫執行，且五個核心要素都與訓練／課程目標切合。',['12a 學員遴選','12b 教材選擇','12c 師資遴選','12d 教學方法','12e 教學環境與設備'], '展開五個子項，再看 clean SAMPLE-002 與 TEST 報名來源。'],
  ['13','確認有提供學員／客戶把所學移轉到工作或實務的建議、機制或協助，並能留下成果資料。',[], '由 clean SAMPLE-002 與 30 日追蹤示範移轉／追蹤路徑。'],
  ['14','確認訓練資料有系統化分類、編碼、建檔、保存、分析、運用與查詢／檢索機制。',[], '由架構文件與 Observation／Evidence locator 示範「找得到、追得回」。'],
  ['15','確認有定期課後檢討、結案／評估報告與綜合分析，並把回饋轉成改善。',[], '調閱 clean SAMPLE-002 方法，再接反應評估 Observation。'],
  ...
  ['18','確認目標客戶及學員對訓練功能／效果的評價具有具體紀錄與適當評量。',[], '目前僅 TEST／SAMPLE 方法；不得把滿意度直接等同正式績效。'],
  ['19','確認目標市場／顧客是否能提出可驗證的價值創造或市場功能證據。',[], '目前無 REAL 市場價值；僅展示方法與缺口狀態。']
];

var TTQS_EXTERNAL_CAUSAL_ = [
  ['1','需求蒐集','7','EV-RUN-A1788F62F85532C2；EV-S002-001','TEST／SAMPLE','可追溯','由真實 Google Forms TEST 提交與 clean SAMPLE-002 情境示範需求來源；不是 REAL 市場需求。'],
  ['2','需求／職能落差分析','7、8','EV-S002-001','TEST／SAMPLE','可追溯','clean SAMPLE-002 落差分析連到訓練目標與方案設計；T Gate 前不得據此鎖定 REAL 課程。'],
  ['3','課程設計／目標／審查','9、10、11','EV-S002-001；EV-S002-002','TEST／SAMPLE','可追溯','利益關係人、資源甄選與需求→目標→評量追溯均為 clean SAMPLE-002 情境。'],
  ['4','執行／資源／班次','12、13、14','EV-S002-003；EV-RUN-D72493E5294CD5B2','TEST／SAMPLE','可追溯','班次與資源為 clean SAMPLE-002；另含真實 Google Forms TEST 報名 submission 的去識別定位。'],
  ['5','評量／檢討','15、16、17','EV-S002-003；EV-RUN-574145BB8CD6BF6F；EV-RUN-REC-CF43A0C33DC3EACD','TEST／SAMPLE','可追溯','反應評估、系統異常控制與 clean SAMPLE-002 方法形成 Review/Outcome 示範；不代表正式成果。'],
  ['6','追蹤／改善','18、19','EV-S002-003；EV-RUN-99545B2AB6B10D3F','TEST／SAMPLE','可追溯','30日追蹤與成果方法只供 TEST／SAMPLE；正式 17–19 成果仍待 REAL 營運。']
];
...
var TTQS_EXTERNAL_EVIDENCE_ = [
  ...
  ['8','EV-S002-001','clean SAMPLE-002 訓練方案設計鏈','SUPPORTS','SAMPLE','STRUCTURED_DATA','CourseVersion','SAMPLE-CV-002','','','','需求→目標→內容／方法→評量的 CLEAN_FIXTURE_2 示範。'],
  ['9','EV-S002-002','clean SAMPLE-002 利益關係人／模擬核准方法','SUPPORTS','SAMPLE','STRUCTURED_DATA','ApprovalEvent','S-AP-004','','','','全部參與者與決策均為 SAMPLE；不冒充真人會議或核准。'],
  ['10','EV-S002-002','clean SAMPLE-002 資源甄選與核准方法','SUPPORTS','SAMPLE','STRUCTURED_DATA','ApprovalEvent','S-AP-004','','','','SAMPLE 方法；受控教材／個資告知／招生快照原件仍有缺口。'],
  ['11','EV-S002-001','clean SAMPLE-002 需求→目標→內容連結','SUPPORTS','SAMPLE','STRUCTURED_DATA','CourseVersion','SAMPLE-CV-002','','','','需求與訓練目標結合的 clean fixture。'],
  ['11','EV-RUN-A1788F62F85532C2','TEST runtime needs response','SUPPORTS','SAMPLE','STRUCTURED_DATA','SurveyResponse','S-RUN-SUR-A1788F62F85532','OBS-5DF457DAE87CE85418F35E8B','SHEET:1145488986:ROW:5','','TEST needs submission；只公開不可逆識別資訊與 locator，不公開回覆內容。'],
  ['12','EV-S002-003','clean SAMPLE-002 執行／資源／班次方法','SUPPORTS','SAMPLE','STRUCTURED_DATA','ClassRun','SAMPLE-CLASS-002','','','','CLEAN_FIXTURE_2 班次情境；非正式班次。'],
  ['12','EV-RUN-D72493E5294CD5B2','TEST runtime registration response','SUPPORTS','SAMPLE','STRUCTURED_DATA','SurveyResponse','S-RUN-SUR-D72493E5294CD5','OBS-0BDC55AF857F2A31861AF73F','SHEET:1407831401:ROW:7','','真實 Google Forms TEST 報名 submission 去識別定位；ACCEPTED / SCHEDULER_PROCESSED。'],
  ['13','EV-S002-003','clean SAMPLE-002 成果移轉方法','SUPPORTS','SAMPLE','STRUCTURED_DATA','ClassRun','SAMPLE-CLASS-002','','','','成果移轉原件尚待建立；此列只示範 clean fixture 方法。'],
  ['13','EV-RUN-99545B2AB6B10D3F','TEST runtime 30-day behavior response','SUPPORTS','SAMPLE','STRUCTURED_DATA','SurveyResponse','S-RUN-SUR-99545B2AB6B10D','OBS-B6FCD3304656E8618BD552CA','SHEET:332084943:ROW:2','','真實 Google Forms TEST 30日追蹤 submission 去識別定位；ACCEPTED。'],
  ...
  ['15','EV-S002-003','clean SAMPLE-002 結案／綜合分析方法缺口','SUPPORTS','SAMPLE','STRUCTURED_DATA','ClassRun','SAMPLE-CLASS-002','','','','正式結案原件尚未形成；不把缺口冒充成果。'],
  ['15','EV-RUN-574145BB8CD6BF6F','TEST runtime reaction response','SUPPORTS','SAMPLE','STRUCTURED_DATA','SurveyResponse','S-RUN-SUR-574145BB8CD6BF','OBS-05BA7E0619D4E63D86151795','SHEET:480064116:ROW:3','','真實 Google Forms TEST 反應 submission 去識別定位；ACCEPTED。'],
  ['16','EV-RUN-REC-CF43A0C33DC3EACD','TEST S3 故障→重試→對帳 CONTROL trace','SUPPORTS','CONTROL','STRUCTURED_DATA','AdminAuditLog','EV-RUN-REC-CF43A0C33DC3EACD','','','','只證明系統控制能力，不是辦訓異常事證。'],
  ['17','EV-S002-003','clean SAMPLE-002 成果評估方法','SUPPORTS','SAMPLE','STRUCTURED_DATA','ClassRun','SAMPLE-CLASS-002','','','','只示範方法；協會尚無 REAL Outcome。'],
  ['17','EV-RUN-574145BB8CD6BF6F','TEST runtime reaction response','SUPPORTS','SAMPLE','STRUCTURED_DATA','SurveyResponse','S-RUN-SUR-574145BB8CD6BF','OBS-05BA7E0619D4E63D86151795','SHEET:480064116:ROW:3','','TEST 反應評估來源；不可作正式成果。'],
  ['18','EV-RUN-99545B2AB6B10D3F','TEST runtime 30-day behavior response','SUPPORTS','SAMPLE','STRUCTURED_DATA','SurveyResponse','S-RUN-SUR-99545B2AB6B10D','OBS-B6FCD3304656E8618BD552CA','SHEET:332084943:ROW:2','','TEST 30日追蹤；正式目標客戶／學員評價待 REAL。'],
...
function artifactId_(id){return 'FA-DEMO-IND-'+('0'+id).slice(-2)+'-R1';}
function artifactHtml_(id){var r=findIndicator_(id);if(!r)return '<h1>Artifact not found</h1>';var adm=r[3]==='FORMAL_BLOCKED_NEEDS_REAL'?'NO_REAL_EVENT':(r[3]==='EVIDENCE_MISSING'?'GAP':'TEST_SAMPLE_CONTROL_ONLY');return '<!doctype html><html lang="zh-Hant-TW"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+artifactId_(id)+'</title><style>'+css_()+'</style></head><body><article class="r3panel"><div><b>DEMO FrozenArtifact projection · TEST/SAMPLE/CONTROL · EXTERNAL_READONLY</b></div><h1>'+artifactId_(id)+'</h1><h2>指標 '+esc_(r[0])+'｜'+esc_(r[2])+'</h2><table class="r3table"><tr><th>release_id</th><td>'+TTQS_RELEASE_+'</td></tr><tr><th>source object</th><td>'+esc_(r[4])+'</td></tr><tr><th>formal_admissibility</th><td>'+adm+'</td></tr><tr><th>boundary / gap</th><td>'+esc_(r[5])+'</td></tr><tr><th>release_data_cutoff_at</th><td>2026-08-19T17:15:00+08:00</td></tr><tr><th>REAL</th><td>NO — 本 artifact 不構成 REAL 辦訓事證或正式 TTQS 評分</td></tr></table><p>此頁是 T_TEST_RELEASE 的固定靜態查驗投影，用來驗證「指標→Matrix→FrozenArtifact」路徑；正式 EvaluationRelease 必須改由受控 frozen export＋hash 形成。</p><a class="button" href="?">返回評核入口</a></article></body></html>';}
function checklistHtml_(){return TTQS_CHECKLIST_.map(function(r){return '<tr><td><b>'+r[0]+'</b></td><td>'+esc_(r[1])+'</td><td>'+esc_(r[2])+'</td><td>'+esc_(r[3])+'</td></tr>';}).join('');}
