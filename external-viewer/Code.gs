...
var TTQS_EXTERNAL_SNAPSHOT_TITLE_ = 'TTQS ONE 外部唯讀 Snapshot（TEST／SAMPLE）';
...
function ttqsExternalRender_(model) {
  ...
  return '<!doctype html><html lang="zh-Hant-TW"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    ...
    '</style></head><body><main><header><div class="eyebrow">TTQS ONE · TEST / SAMPLE · 外部唯讀</div><h1>' + ttqsExternalEscape_(model.title) + '</h1>' +
    ...
    '<section class="grid">' + cards + '</section><p class="foot">本 Viewer 僅讀取去識別 Snapshot，不直接連線 TTQS ONE 核心資料庫，也不提供新增、修改、刪除、核准、REAL Start 或背景工作控制功能。</p>' +
    '</main></body></html>';
}
