...
var TTQS_R7_RELEASE_ID_='ER-DEMO-20260901-DRAFT-003';
var TTQS_R7_DATA_CLASS_='TEST/SAMPLE/CONTROL';
var TTQS_R7_PROJECTION_RAW_SHA256_='94590a9bbfdca699235815fb96e4c37c69156f10689e70b6c3caa74527165a53';
var TTQS_R7_MANIFEST_SHA256_='e9e5e2145e915a5eac53905239ce52e25b9ea90911756bc707821f0ec568dd79';
var TTQS_R7_OFFLINE_ZIP_SHA256_='8b79687329b03c08e971cde0ccd8f8efd312487543e2d285e05f4838b1cc3059';
...
function ttqsR7Focus_(id){for(var i=0;i<TTQS_EXTERNAL_OFFICIAL_FOCUS_.length;i++)if(String(TTQS_EXTERNAL_OFFICIAL_FOCUS_[i][0])===String(id))return TTQS_EXTERNAL_OFFICIAL_FOCUS_[i];return [String(id),'依現行訓練機構版查核佐證文件資料表逐項調閱。',[],''];}
...
function ttqsR7Warning_(){return '<div class="r7warn"><b>⚠️ 模擬評核查驗｜TEST／SAMPLE／CONTROL｜非正式事證</b><br>協會目前沒有 REAL 辦訓課程、學員、講師、成果或 Outcome。本頁只驗證 TTQS ONE 的 DEMO 證據架構與查驗路徑；不得用於正式 TTQS 評分、牌等推論或官方送件。<br><b>129</b> 是依官方表「常見參考佐證資料」逐條拆分的 TTQS ONE 內部加嚴 DEMO 覆蓋數；官方原文為「以下為例，但不以此為限」，並非官方強制 129 份文件。<br><b>開啟提醒：</b>如果用 Chrome 且登入多個 Google 帳號時看到錯誤，請改用無痕視窗或 Safari 私密瀏覽開啟。</div>';}

function ttqsR7HomeHtml_(){
  var data=ttqsR7Data_(),cards=[],countTotal=0;
  for(var i=1;i<=19;i++){
...
function ttqsR7MatrixHtml_(indicator){
  var id=String(indicator||''),items=ttqsR7IndicatorItems_(id);if(!items.length)return ttqsR7ErrorHtml_('找不到指定指標','indicator='+id);
  ...
  return ttqsR7Head_('TTQS ONE｜指標 '+id+' Evidence Matrix')+ttqsR7Warning_()+'<section class="r3panel" data-matrix-indicator="'+esc_(id)+'"><div class="r7bar"><a class="r7button secondary" href="'+esc_(TTQS_R3_CANONICAL_EXEC_URL_)+'">← 回首頁</a></div><h1>指標 '+esc_(id)+'｜Evidence Matrix</h1><p>'+esc_(focus[1])+'</p><p><b>正式可評事證：</b>'+esc_(st.formal)+'</p><p><b>本指標 DEMO 細項：</b>'+items.length+'；每列均有獨立 artifact code、PDF SHA-256、文字投影 SHA-256 與 Offline 相對路徑。</p>'+sections+'</section>'+ttqsR7Foot_();
}
function ttqsR7ArtifactHtml_(artifactCode){
  var requested=String(artifactCode||'');if(requested==='FA-DEMO-002')return ttqsR3ArtifactHtml_(requested);
